...
/// Parent suite serializing all URLSession tests to avoid
/// StubURLProtocol handler conflicts.
@Suite("URLSessionHTTPClient", .serialized)
enum URLSessionHTTPClientAllTests {
    ...
    struct CoreTests {
        ...
        func requestStatusAndBody() async throws {
            ...
            #expect(
                String(
                    data: response.body, encoding: .utf8
                ) == "hello"
            )
        }
        ...
        func uploadReturnsResponse() async throws {
            ...
            let url = try #require(
                URL(
                    string: "https://test.local/upload"
                )
            )
            let tempFile =
